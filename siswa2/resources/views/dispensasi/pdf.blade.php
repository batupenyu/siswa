    ...
    <table>
        ...
        <tr style="vertical-align: top">
            ...
            <td>
                ...
                @else
                    <a href="{{ route('tabeldispensasi', $dispensasi->id) }}" class="btn btn-primary btn-sm"><i>Daftar nama terlampir</i></a>
                @endif
            </td>
        </tr>
        <tr style="vertical-align: top">
            ...
            <td>
                ...
                @if ($dispensasi->tgl_kegiatan == $dispensasi->tgl_akhir_kegiatan)
                    {{ Carbon\carbon::parse($dispensasi->tgl_kegiatan)->translatedFormat('l') }} - {{ Carbon\carbon::parse($dispensasi->tgl_akhir_kegiatan)->translatedFormat('l') }}/ {{ Carbon\carbon::parse($dispensasi->tgl_kegiatan)->translatedFormat('d-m-Y') }} s.d. {{ Carbon\carbon::parse($dispensasi->tgl_akhir_kegiatan)->translatedFormat('d-m-Y') }} <br>
                @else
                    {{ Carbon\carbon::parse($dispensasi->tgl_kegiatan)->translatedFormat('l') }}/  {{ Carbon\carbon::parse($dispensasi->tgl_akhir_kegiatan)->translatedFormat('d-m-Y') }} <br>
                @endif
                
                Pukul <span style="padding-left: 43px">:</span> {{ Carbon\Carbon::parse($dispensasi->jam_kegiatan )->format('H:i')}} WIB s.d Selesai
                <br>
