...
@section('content')
    <h4>Foto Kegiatan : {{ $st_surat->nama_kegiatan ?? '' }} Tanggal {{ \Carbon\Carbon::parse($st_surat->tgl_kegiatan)->format('d-m-Y') ?? '' }} s.d. {{ \Carbon\Carbon::parse($st_surat->tgl_akhir_kegiatan)->format('d-m-Y') ?? '' }}</h4>
    @if ($photo->isEmpty())
