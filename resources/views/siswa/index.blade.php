...
<div class="container mt-5">
    ...
    <table class="table table-sm table-bordered table-striped">
        ...
        <tbody>
            ...
            <tr>
                <td style="text-align:center">{{ ($siswas->currentPage()-1) * $siswas->perPage() + $loop->index + 1 }}.</td>
                <td>{{ $siswa->name }}</td>
                <td>{{ $siswa->nis }}</td>
                    ...
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                            ...
                            <form action="{{ route('siswas.destroy', $siswa->id) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="dropdown-item" onclick="return confirm('Apakah Anda yakin ingin menghapus siswa ini?')">Hapus</button>
                            </form>
                        </div>
                    </div>
                </td>
